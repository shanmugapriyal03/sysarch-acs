...
/**
  ...
  @return  None
**/
void pal_pci_cfg_write(uint32_t seg, uint32_t bus, uint32_t dev, uint32_t func, uint32_t offset,
                       uint32_t data)
{
...
**/
static void
get_resource_base_32(uint32_t seg, uint32_t bus, uint32_t dev, uint32_t func, uint32_t bar32_p_base,
                     uint32_t bar32_np_base, uint32_t bar32_p_limit, uint32_t bar32_np_limit)
{
       ...
       ((bar32_np_base == g_bar32_np_start) && (g_np_bar_size != 0)))
  {
      ...
      pal_pci_cfg_read(seg, bus, dev, func, NON_PRE_FET_OFFSET, &mem_bar_np);
      mem_bar_np = ((bar32_np_limit & MEM_BASE32_LIM_MASK) | mem_bar_np);
      pal_pci_cfg_write(seg, bus, dev, func, NON_PRE_FET_OFFSET, mem_bar_np);
   }
      ...
      ((bar32_p_base == g_bar32_p_start) && (g_p_bar_size != 0)))
  {
      ...
      pal_pci_cfg_read(seg, bus, dev, func, PRE_FET_OFFSET, &mem_bar_p);
...
**/
static void
get_resource_base_64(uint32_t seg, uint32_t bus, uint32_t dev, uint32_t func,
...
void
pal_pcie_rp_program_bar(uint32_t seg, uint32_t bus, uint32_t dev, uint32_t func)
{
...
/**
  ...
  @return  None
**/
void pal_pcie_program_bar_reg(uint32_t seg, uint32_t bus, uint32_t dev, uint32_t func)
{
  ...
  while(offset <= TYPE0_BAR_MAX_OFF)
  {
    ...
    else
    {
         ...
         **/
         ...
         if (BAR_REG(bar_reg_value) == BAR_64_BIT)
         {
             pal_pci_cfg_write(seg, bus, dev, func, offset + 4, 0);
             offset = offset + 8;
         }
  ...
  for (dev = 0; dev < PCIE_MAX_DEV; dev++)
  {
    for (func = 0; func < PCIE_MAX_FUNC; func++)
    {
        ...
        if (PCIE_HEADER_TYPE(header_value) == TYPE1_HEADER)
        {
            ...
            pal_pci_cfg_write(seg, bus, dev, func, NON_PRE_FET_OFFSET,
                              ((g_bar32_np_start >> 16) & 0xFFF0));
            pal_pci_cfg_write(seg, bus, dev, func, PRE_FET_OFFSET,
                              ((g_bar32_p_start >> 16) & 0xFFF0));
            sub_bus = pal_pcie_enumerate_device(sec_bus, (sec_bus+1));
            pal_pci_cfg_write(seg, bus, dev, func, BUS_NUM_REG_OFFSET,
                              BUS_NUM_REG_CFG(sub_bus, sec_bus, bus));
            sec_bus = sub_bus + 1;
            ...
            get_resource_base_32(seg, bus, dev, func, bar32_p_base, bar32_np_base,
                                 bar32_p_limit, bar32_np_limit);

            /*Obtain the start memory base address and the final memory base address of 64 bit BAR*/
            get_resource_base_64(seg, bus, dev, func, bar64_p_base, g_bar64_p_max);


            pal_pcie_rp_program_bar(seg, bus, dev, func);
    ...
    for (bus = 0; bus <= g_pcie_info_table->block[pcie_index].end_bus_num; bus++)
    {
        for (dev = 0; dev < PCIE_MAX_DEV; dev++)
        {
            for (func = 0; func < PCIE_MAX_FUNC; func++)
            {
                ...
                if (PCIE_HEADER_TYPE(header_value) == TYPE1_HEADER)
                {
                    pal_pci_cfg_read(seg, bus, dev, func, BUS_NUM_REG_OFFSET, &bus_value);
                    bus_value = bus_value & PRI_BUS_CLEAR_MASK;
                    pal_pci_cfg_write(seg, bus, dev, func, BUS_NUM_REG_OFFSET, bus_value);
                }
            }
         }
     }
...
void pal_pcie_enumerate(void)
{
    ...
    while (pcie_index < g_pcie_info_table->num_entries)
    {
       ...
       while (count < hb_count)
       {
           ...
           g_bar32_p_start  = platform_root_pcie_cfg.block[pcie_index].ep_pbar32_value[count];
           g_rp_bar32_value = platform_root_pcie_cfg.block[pcie_index].rp_bar32_value[count];

           pri_bus = platform_root_pcie_cfg.block[pcie_index].start_bus_num[count];
           sec_bus = pri_bus + 1;
      ...
      for (Func = InputFunc; Func < PCIE_MAX_FUNC; Func++)
      {
        ...
        if ((class_code >> CC_BASE_SHIFT) == (ClassCode >> 16))
        {
          if((class_code >> CC_SUB_SHIFT) == (ClassCode >> 8))
          {
           // Found our device
           // Return the BDF*/
            return (uint32_t)(PCIE_CREATE_BDF(InputSeg, Bus, Dev, Func));
          }
        }
      }
    }
...
pal_pcie_get_base(uint32_t bdf, uint32_t bar_index)
{
  ...
  pal_pci_cfg_read(Seg, Bus, Dev, Func, offset, &bar_reg_lower_value);

  bar_value = bar_reg_lower_value & BAR_MASK;

  if (BAR_REG(bar_reg_lower_value) == BAR_64_BIT)
  {
     pal_pci_cfg_read(Seg, Bus, Dev, Func, offset + 4, &bar_reg_upper_value);
