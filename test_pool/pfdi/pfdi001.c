...
#define TEST_RULE  "R0053, R0055, R0104"
#define TEST_DESC  "Check PFDI Version in All PE's            "

extern int64_t g_pfdi_version;
...
static void payload(uint32_t num_pe)
{
  ...
  /* Invoke PFDI Version function for current PE index */
  g_pfdi_version = val_pfdi_version();
  if (g_pfdi_version >> 31) {
    val_print(ACS_PRINT_ERR,
                "\n       PFDI get version failed err = %d", g_pfdi_version);
  ...
  if (val_pfdi_reserved_bits_check_is_zero(
             VAL_EXTRACT_BITS(g_pfdi_version, 31, 63)) != ACS_STATUS_PASS) {
    val_print(ACS_PRINT_ERR, "\n       Failed on PE = %d", index);
    val_set_status(index, RESULT_FAIL(TEST_NUM, 2));
  }

  major = PFDI_VERSION_GET_MAJOR(g_pfdi_version);
  ...
  minor = PFDI_VERSION_GET_MINOR(g_pfdi_version);
  ...
  for (i = 0; i < num_pe; i++) {
    if (i != index) {
      ...
      if (version >> 31) {
        val_print(ACS_PRINT_ERR,
                "\n       PFDI get version failed err = %d", version);
        val_print(ACS_PRINT_ERR, "\n       Failed on PE = %d", i);
      ...
      if (minor != PFDI_MINOR_VERSION) {
        val_print(ACS_PRINT_ERR,
                "\n       Minor Version not as expected, Current version =%d", minor);
        val_print(ACS_PRINT_ERR, "\n       Failed on PE = %d", i);
        val_set_status(i, RESULT_FAIL(TEST_NUM, 9));
        goto free_pfdi_details;
      }
      ...
      if (val_pfdi_reserved_bits_check_is_zero(
        ...
        val_print(ACS_PRINT_ERR, "\n       Failed on PE = %d", i);
        val_set_status(i, RESULT_FAIL(TEST_NUM, 10));
        goto free_pfdi_details;
      }
    }
  }
