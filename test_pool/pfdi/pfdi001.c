...
static void payload_version_check(void *arg)
{
  ...
  pfdi_version = val_pfdi_version();
  if (pfdi_version >> 31) {
    val_print(ACS_PRINT_ERR,
                "\n       PFDI get version failed err = %ld", pfdi_version);
  ...
  if (val_pfdi_reserved_bits_check_is_zero(
             VAL_EXTRACT_BITS(pfdi_version, 31, 63)) != ACS_STATUS_PASS) {
    val_print(ACS_PRINT_ERR, "\n       Failed on PE = %d", index);
    val_set_status(index, RESULT_FAIL(TEST_NUM, 2));
  }

  major = PFDI_VERSION_GET_MAJOR(pfdi_version);
  ...
  minor = PFDI_VERSION_GET_MINOR(pfdi_version);
