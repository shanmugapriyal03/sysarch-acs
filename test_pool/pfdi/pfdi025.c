...
#define TEST_NUM   (ACS_PFDI_TEST_NUM_BASE + 25)
#define TEST_RULE  "R0172"
#define TEST_DESC  "PFDI PE Test Result invalid params check  "
...
PFDI_INVAL_RETURNS *g_pfdi_invalid_test_result;

void
pfdi_invalid_test_result_check(void)
{
  uint32_t index = val_pe_get_index_mpid(val_pe_get_mpid()), i;
  PFDI_INVAL_RETURNS *pfdi_buffer;

  pfdi_buffer = g_pfdi_invalid_test_result + index;

  /* Invoke PFDI Test Result function for current PE index with invalid x1 parameter*/
  pfdi_buffer->inval[0].x0 = val_invoke_pfdi_fn(
            PFDI_FN_PFDI_PE_TEST_RESULT, PFDI_INVALID_X_VALUE, 0, 0, 0, 0,
  ...
  /* Invoke PFDI Test Result function for current PE index with invalid x2 parameter*/
  pfdi_buffer->inval[1].x0 = val_invoke_pfdi_fn(
            PFDI_FN_PFDI_PE_TEST_RESULT, 0, PFDI_INVALID_X_VALUE, 0, 0, 0,
  ...
  /* Invoke PFDI Test Result function for current PE index with invalid x3 parameter*/
  pfdi_buffer->inval[2].x0 = val_invoke_pfdi_fn(
            PFDI_FN_PFDI_PE_TEST_RESULT, 0, 0, PFDI_INVALID_X_VALUE, 0, 0,
   ...
   /* Invoke PFDI Test Result function for current PE index with invalid x4 parameter*/
  pfdi_buffer->inval[3].x0 = val_invoke_pfdi_fn(
            PFDI_FN_PFDI_PE_TEST_RESULT, 0, 0, 0, PFDI_INVALID_X_VALUE, 0,
...
static void payload_invalid_pe_test_result_check(void *arg)
{
  ...
  /* Allocate memory to save all PFDI Test Results or status for all PE's */
  ...
  for (i = 0; i < num_pe; i++) {
    pfdi_buffer = g_pfdi_invalid_test_result + i;
    for (num_regs = 0; num_regs < NUM_REGS_TO_CHECK; num_regs++) {
      val_pfdi_invalidate_ret_params(&pfdi_buffer->inval[num_regs]);
    }
  }

  /* Invoke pfdi_invalid_test_result_check for current PE index */
  pfdi_invalid_test_result_check();

  /* Execute pfdi_invalid_test_result_check function in All PE's */
  for (i = 0; i < num_pe; i++) {
    if (i != index) {
      timeout = TIMEOUT_LARGE;
      val_execute_on_pe(i, pfdi_invalid_test_result_check, (uint64_t)g_pfdi_invalid_test_result);
  ...
  for (i = 0; i < num_pe; i++) {
    ...
    for (inval_case = 0; inval_case < NUM_REGS_TO_CHECK; inval_case++) {
      ...
      if (pfdi_buffer->inval[inval_case].x0 != PFDI_ACS_INVALID_PARAMETERS) {
        val_print(ACS_PRINT_ERR,
           "\n       PFDI test_result with invalid x%d", inval_case + 1);
...
free_pfdi_details:
  val_memory_free((void *) g_pfdi_invalid_test_result);
...
uint32_t pfdi025_entry(uint32_t num_pe)
{
  ...
  if (status != ACS_STATUS_SKIP)
    val_run_test_configurable_payload(&num_pe, payload_invalid_pe_test_result_check);
