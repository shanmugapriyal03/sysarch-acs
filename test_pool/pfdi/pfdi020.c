...
#define TEST_NUM   (ACS_PFDI_TEST_NUM_BASE + 20)
#define TEST_RULE  "R0168"
#define TEST_DESC  "Check PE Run with Start or End less -1    "

static PFDI_RET_PARAMS *g_pfdi_status;

/* Execute invalid parameter test cases:
 * Case 0: Start < -1
 * Case 1: End < -1
 */
static void
check_pe_test_run_less_than_minus_one(void)
{
    uint32_t index;
    int64_t test_parts;
    PFDI_RET_PARAMS *pfdi_buffer_case0, *pfdi_buffer_case1;

    index = val_pe_get_index_mpid(val_pe_get_mpid());
    pfdi_buffer_case0 = g_pfdi_status + index;
    pfdi_buffer_case1 = g_pfdi_status + val_pe_get_num() + index;

    /* Query number of supported test parts */
    ...
    /* Case 0: Start < -1 */
    pfdi_buffer_case0->x0 = val_pfdi_pe_test_run(-2, test_parts - 1,
                             &pfdi_buffer_case0->x1, &pfdi_buffer_case0->x2,
                             &pfdi_buffer_case0->x3, &pfdi_buffer_case0->x4);
    val_pfdi_invalidate_ret_params(pfdi_buffer_case0);

    /* Case 1: End < -1 */
    pfdi_buffer_case1->x0 = val_pfdi_pe_test_run(0, -2,
                             &pfdi_buffer_case1->x1, &pfdi_buffer_case1->x2,
...
/* Validate that PFDI_PE_TEST_RUN returns INVALID_PARAMETERS for < -1 inputs */
static void
payload_check_pe_test_run_less_than_minus_one(void *arg)
{
    ...
    /* Allocate memory for 2 cases * num_pe */
    ...
    check_pe_test_run_less_than_minus_one();

    for (i = 0; i < num_pe; i++) {
        if (i != index) {
            timeout = TIMEOUT_LARGE;
            val_execute_on_pe(i, check_pe_test_run_less_than_minus_one, 0);
    ...
    for (i = 0; i < num_pe; i++) {
        ...
        for (j = 0; j < 2; j++) {
            pfdi_buffer = g_pfdi_status + (j * num_pe) + i;
            ...
            if (pfdi_buffer->x0 != PFDI_ACS_INVALID_PARAMETERS) {
                ...
                val_print(ACS_PRINT_ERR, " when %a",
                          (uint64_t)(j == 0 ? "Start is less than -1" :
                                              "End is less than -1"));
...
uint32_t pfdi020_entry(uint32_t num_pe)
{
    ...
    if (status != ACS_STATUS_SKIP)
        val_run_test_configurable_payload(&num_pe, payload_check_pe_test_run_less_than_minus_one);
