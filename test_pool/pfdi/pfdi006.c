...
pfdi_test_results(void)
{
  ...
  status = val_pfdi_pe_test_result(&fault_test_id);

  status_buffer->results_status = status;
...
static void payload_test_results(void *arg)
{
  ...
  if (status < PFDI_ACS_SUCCESS) {
    if (status == PFDI_ACS_FAULT_FOUND) {
      ...
      val_print(ACS_PRINT_ERR,
               "\n       PFDI query PE Results function failed err = %d", status);
    }
    val_set_status(index, RESULT_FAIL(TEST_NUM, 1));
    return;
  }
    ...
    if (i != index) {
      ...
      if (status_buffer->results_status < PFDI_ACS_SUCCESS) {
        if (status_buffer->results_status == PFDI_ACS_FAULT_FOUND) {
          ...
          val_print(ACS_PRINT_ERR, "\n       PFDI query PE Results function failed err = %d",
