...
#define TEST_NUM   (ACS_GIC_ITS_TEST_NUM_BASE + 5)
#define TEST_RULE  "ITS_DEV_8"
#define TEST_DESC  "Check Device IDs not behind SMMU      "
...
payload()
{
  ...
  for (tbl_index = 0; tbl_index < bdf_tbl_ptr->num_entries; tbl_index++)
  {
    ...
    if (smmu_id != ACS_INVALID_INDEX) {
        /* Skip the device if it is behind SMMU */
        continue;
    }

    /* If test runs for atleast one device */
    test_skip = 0;
