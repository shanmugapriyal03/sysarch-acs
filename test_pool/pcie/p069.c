...
test_config_t test_entries[] = {
        { ACS_PCIE_TEST_NUM_BASE + 69, "Check MSI and MSI-X support: RCiEP    ", "RE_INT_1"},
        { ACS_PCIE_TEST_NUM_BASE + 55, "Check MSI and MSI-X support: iEP/RP   ", "IE_INT_1"}
    };
      ...
      if ((dp_type == test_data->dev_type1) || (dp_type == test_data->dev_type2))
      {
         ...
         if ((val_pcie_find_capability(bdf, PCIE_CAP, CID_MSI, &cap_base) == PCIE_CAP_NOT_FOUND)
           && (val_pcie_find_capability(bdf, PCIE_CAP, CID_MSIX, &cap_base) == PCIE_CAP_NOT_FOUND)
             && ((int_pin >= 1) && (int_pin <= 4))) {
              val_print(ACS_PRINT_ERR, "\n       BDF - 0x%x supports INTx but not MSI/MSI-X", bdf);
              test_fails++;
             }
