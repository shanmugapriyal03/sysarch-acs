...
#define TEST_NUM   ACS_MPAM_CACHE_TEST_NUM_BASE + 2
#define TEST_RULE  ""
...
static void payload(void)
{
    ...
    uint32_t test_partid = 3; //Selecting distinct partid
   ...
   /* Get the Index for LLC */
    ...
    if ((ccap_nodes == 0) || (csumon_count == 0)) {
        val_set_status(index, RESULT_SKIP(TEST_NUM, 03));
        return;
    }

    /* Dynamically create the CSU Mon Counter buffer
     * uint64_t counter[CCAP_SCENARIO_MAX][ccap_nodes]
     */
    counter = (uint64_t **) val_memory_alloc(CCAP_SCENARIO_MAX * sizeof (uint64_t *));
    for (it = 0; it < CCAP_SCENARIO_MAX; it++) {
    ...
    for (cfg_index = 0; cfg_index < sizeof(ccap_config_data)/sizeof(ccap_config_t); cfg_index++) {
      ...
      if (ccap_config_data[cfg_index].config_enable) {
        /* Configure CCAP settings for nodes supporting CCAP */
        for (msc_index = 0; msc_index < msc_node_cnt; msc_index++) {
          ...
          for (rsrc_index = 0; rsrc_index < rsrc_node_cnt; rsrc_index++) {
            ...
            if (val_mpam_supports_ccap(msc_index)) {
              val_mpam_configure_ccap(msc_index, test_partid,
                                      SOFTLIMIT_DIS,
                                      ccap_config_data[cfg_index].partition_percent);
            }

            if (val_mpam_supports_cpor(msc_index))
              val_mpam_configure_cpor(msc_index, test_partid, 100);

            buf_size = cache_maxsize * ccap_config_data[cfg_index].cache_percent / 100 / 2;
