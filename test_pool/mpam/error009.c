...
#define TEST_NUM   ACS_MPAM_ERROR_TEST_NUM_BASE  +  9
#define TEST_DESC  "Check RIS no control error            "
#define TEST_RULE  ""

#define PSUEDO_REG_VALUE 0xFFFF
...
static
uint32_t
configure_partitioning_reg(uint32_t reg_offset)
{
    uint32_t esr_errcode;
    uint32_t status;

    /* Write 0xFFFF to the unimplemented partitioning register of the MSC's resource */
    val_mpam_mmr_write(msc_index, reg_offset, PSUEDO_REG_VALUE);
    ...
    if (esr_errcode != ESR_ERRCODE_RIS_NO_CTRL)
    {
        val_print(ACS_PRINT_ERR, "\n       Expected errcode: %d", ESR_ERRCODE_RIS_NO_CTRL);
...
static
uint32_t
check_for_rsc_part_controls(void)
{
    uint32_t status;

    if (!val_mpam_supports_ccap(msc_index)) {
        /* If CCAP is not implemented by the Resource, access MPAMCFG_CMAX to trigger the error */
        status = configure_partitioning_reg(REG_MPAMCFG_CMAX);
        return status;
    }
    else
    if (!val_mpam_supports_cpor(msc_index)) {
        /* If CPOR is not implemented by the Resource, access MPAMCFG_CPBM to trigger the error */
        status = configure_partitioning_reg(REG_MPAMCFG_CPBM);
        return status;
    }
    else
    if (!val_mpam_msc_supports_mbwpart(msc_index)) {
         /* If MBW Partitioning is not implemented by the Resource, access MPAMCFG_MBW*
            to trigger the error */
         status = configure_partitioning_reg(REG_MPAMCFG_MBW_MAX);
         status |= configure_partitioning_reg(REG_MPAMCFG_MBW_MIN);
         status |= configure_partitioning_reg(REG_MPAMCFG_MBW_PBM);
         return status;
    }
...
void payload(void)
{
    ...
    for (msc_index = 0; msc_index < total_nodes; msc_index++) {
        ...
        for (rsc_index = 0; rsc_index < rsc_count; rsc_index++) {
            ...
            /* Configure MPAMCFG_PART_SEL.RIS to select the current RIS Index */
            val_mpam_memory_configure_ris_sel(msc_index, rsc_index);

            /* Check if the resource of the MSC implements partitioning controls and generate the
               error by writing into unimplemented partitioning registers of the MSC resource */
            status = check_for_rsc_part_controls();

            if (status == ACS_STATUS_SKIP) {
                /* If the resource implements all the partitioning controls, skip the rsc.
                   Can't generate the error */
                continue;
            }
            else if (status != ACS_STATUS_PASS) {
                /* If the resource does not implement some partitioning controls and are
