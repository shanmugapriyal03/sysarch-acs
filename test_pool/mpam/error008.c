...
{
    /* With out-of-range RIS value programmed to MPAM_CFG_PART_SEL.RIS, access to MPAMCFG_* should
    ...
    if (!val_mpam_msc_supports_esr(msc_index)) {
        ...
        if (esr_errcode != ESR_ERRCODE_UNDEF_RIS_PART_SEL)
        {
            ...
            if (status == ACS_STATUS_FAIL) {
                val_print(ACS_PRINT_ERR,
                    "\n       MPAMCFG_PART_SEL is not RAZ/WI with out-of-range RIS programmed", 0);
                test_fail++;
            }
        }
