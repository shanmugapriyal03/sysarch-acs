...
#define TEST_NUM   ACS_MPAM_MEMORY_TEST_NUM_BASE  +  3
#define TEST_DESC  "Check MBWMAX Partitioning             "
#define TEST_RULE  ""

#define MBWMAX_SCENARIO_MAX 10
...
mbwmaxconfig_t mbwmax_config_data[] = {
    {"MBWMAX latency check for 20% bw portion size", 20, 30 * SIZE_1G},
    {"MBWMAX latency check for 05% bw portion size", 05, 30 * SIZE_1G},
};
...
get_buffer_size(uint32_t msc_index, uint32_t rsrc_index, uint64_t addr_len, uint32_t index)
{
    ...
    if (membw == HMAT_INVALID_INFO) {
        ...
        buffer_size = mbwmax_config_data[index].memcopy_size;
        val_print(ACS_PRINT_WARN, "  buffer_size: %llx", buffer_size);
        return buffer_size;
    }

    val_print(ACS_PRINT_DEBUG, "\n       Channel BW - %d MB/s", membw);
    limited_bw = membw / (mbwmax_config_data[index].partition_percent);
    ...
    if (index == 0)
        ...
        buffer_size = mbwmax_config_data[0].memcopy_size;
...
void payload(void)
{
    ...
    uint64_t counter[MBWMAX_SCENARIO_MAX][total_nodes][10];

    minmax_partid = DEFAULT_PARTID_MAX;

    /*
     * Compute the number of MBWMAX supported nodes,
    ...
    for (msc_index = 0; msc_index < total_nodes; msc_index++) {
        ...
        for (rsrc_index = 0; rsrc_index < rsrc_node_cnt; rsrc_index++) {
            ...
            /* Select Resource within MSC and read MBWMAX info */
    ...
    val_print(ACS_PRINT_TEST,
        "\n       %d MSC Memory Nodes support MBW Max Limit Partitioning", mbwmax_node_cnt);
    ...
    /* Iterate through various partition settings and gather latency/MBWU monitor count for each */
    for (index = 0; index < sizeof(mbwmax_config_data)/sizeof(mbwmaxconfig_t); index++) {

        val_print(ACS_PRINT_DEBUG,
                        "\n       Programming MSC with %d percent of Max BW Limit partitioning",
                                                mbwmax_config_data[index].partition_percent);
        ...
        for (msc_index = 0; msc_index < total_nodes; msc_index++) {
            ...
            for (rsrc_index = 0; rsrc_index < rsrc_node_cnt; rsrc_index++) {
                ...
                /* Configure MBWMAX partition properties for this MSC */
                val_mpam_msc_configure_mbwmax(msc_index, minmax_partid, HARDLIMIT_EN,
                                          mbwmax_config_data[index].partition_percent);
