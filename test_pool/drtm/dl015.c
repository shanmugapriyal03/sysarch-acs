...
#define TEST_NUM   (ACS_DRTM_DL_TEST_NUM_BASE  +  15)
#define TEST_RULE  "R48030"
#define TEST_DESC  "Validate DLME Authorities event ordering  "
...
payload(uint32_t num_pe)
{
  ...
  dlme_auth = val_drtm_get_feature(DRTM_DRTM_FEATURES_DLME_IMG_AUTH);
  if (dlme_auth != DRTM_DLME_IMG_FEAT_DLME_IMG_AUTH_SUPP) {
    val_print(ERROR,
  ...
  /* Verify if DLME Auth PCR schema is supported. If not, the test is skipped */
  pcr_schema = val_drtm_get_feature(DRTM_DRTM_FEATURES_PCR_SCHEMA);
  if ((pcr_schema & DRTM_PCR_SCHEMA_USAGE_MASK_AUTH_SCHEMA) !=
                        DRTM_TPM_FEAT_PCR_SCHEMA_DLME_AUTH_SUPP) {
    val_print(ERROR,
              "\n       DRTM implementation does not support DLME Auth PCR schema, skip check");
  ...
  /* Request DLME image authentication to use DLME Authorities PCR schema */
  drtm_params->launch_features |= (DRTM_LAUNCH_FEAT_REQ_DLME_IMG_AUTH <<
                    DRTM_LAUNCH_FEAT_SHIFT_DLME_IMG_AUTH);
  ...
  /* Iterate through event log entries and collect PCR[17] and PCR[18] events */
  ...
  /* Validate PCR[18] event sequence for DLME Authorities schema */
  event_idx = 0;
  /* PCR_SCHEMA required */
  ...
  /* DLME_SVN required if auth succeeds and SVN present */
  if (event_idx < pcr18_count && pcr18_events[event_idx] == DRTM_EVTYPE_ARM_DLME_SVN)
    event_idx++;
