...
#define TEST_NUM1  (ACS_RAS_TEST_NUM_BASE + 15)
#define TEST_RULE1 "SYS_RAS_3"
#define TEST_DESC1 "Check Error when Poison unsupported   "
...
ras011_entry(uint32_t num_pe)
{
  ...
  if (status != ACS_STATUS_SKIP)
      val_run_test_payload(TEST_NUM, num_pe, payload_poison_supported, 0);
...
ras015_entry(uint32_t num_pe)
{
  ...
  if (status != ACS_STATUS_SKIP)
      val_run_test_payload(TEST_NUM1, num_pe, payload_poison_unsupported, 0);
